...
namespace Business.Services
{
    public class TableDetailsService : ITableDetailsService
    {
        ...
        private List<ColumnsDetails> GetPostGreSourceColumns(DataBaseSchema dataBaseSchema, string tableName)
        {
            ...
            NpgsqlCommand command = new NpgsqlCommand("SELECT  C.COLUMN_NAME, C.DATA_TYPE, C.CHARACTER_MAXIMUM_LENGTH, U.CONSTRAINT_NAME FROM INFORMATION_SCHEMA.COLUMNS C FULL OUTER JOIN INFORMATION_SCHEMA.KEY_COLUMN_USAGE U ON C.COLUMN_NAME = U.COLUMN_NAME WHERE C.TABLE_NAME = '" + tableName + "'", conn);
        ...
        private List<TablesSchemaDto> GetMySqlSource(DataBaseSchema dataBaseSchema)
        {
            ...
            conn.Open();
            MySqlCommand command = new MySqlCommand("show tables from SAKILA", conn);
            ...
            {
                TablesSchemaDto tablesSchemaDto = new TablesSchemaDto();
                tablesSchemaDto.TableName = dr["Tables_in_SAKILA"].ToString();
                tablesSchemaDto.ColumnsDetails = GetMySqlSourceColumns(dataBaseSchema, dr["Tables_in_SAKILA"].ToString());
                lsttables.Add(tablesSchemaDto);
            }
            conn.Close();
        ...
        private List<ColumnsDetails> GetMySqlSourceColumns(DataBaseSchema dataBaseSchema, string tableName)
        {
            ...
            MySqlCommand command = new MySqlCommand("SELECT  c.column_name,c.data_type,c.character_maximum_length, U.CONSTRAINT_NAME FROM information_schema.columns C left JOIN information_schema.KEY_COLUMN_USAGE U ON C.COLUMN_NAME = U.COLUMN_NAME WHERE C.TABLE_NAME ='" + tableName + "' GROUP BY column_name", conn);
